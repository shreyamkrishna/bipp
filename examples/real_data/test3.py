...
if (3 in plotList):
    ...
    print(f"Max Eigenvalue :{Eigs.max()}, Min Eigenvalue: {(Eigs.min())}")
    print(f"Eigs: {Eigs}")
...
## Simple reshaping DOES NOT work
## 0312 does NOT work
# FoV of datasets (deg): 0.05, 4.26, 11.37, 14.3
# scale of datasets (1024 pixels image; arc seconds): 0.0879, 14.9766, 39.9727, 50.2734 
#wsclean -name WL1000_200MHz_t1 -size 1024 1024 -scale 0.0879asec -multiscale -auto-threshold 1 -auto-mask 3 -mgain 0.8 -niter 1000000 -verbose WL1000_200MHz_0h10m_t1.ms/  2>&1|tee WL1000_200MHz_t1.log
#wsclean -name gauss4_lvl0 -size 1024 1024 -scale 14.9766asec -multiscale -auto-threshold 1 -auto-mask 3 -mgain 0.8 -niter 1000000 -verbose gauss4_t201806301100_SBL180_lvl0.ms/ 2>&1|tee gauss4_lvl0_wsclean.log
...
# For OSKAR generated synthetic data
BIPP_Vis=np.zeros((args.nlevel, 0, N_antenna*(N_antenna-1)//2)) 
# For real MWA data
#BIPP_Vis=np.zeros((args.nlevel, 0, N_antenna*(N_antenna-1)//2 + N_antenna)) 
...
for i in np.arange(Eigs.shape[0]):
    ...
    #For OSKAR generated synthetic data
    BIPP_Vis_levels = np.zeros((0, N_antenna*(N_antenna-1)//2)) 
    # For real MWA data
    #BIPP_Vis_levels = np.zeros((0, N_antenna*(N_antenna-1)//2 + N_antenna)) 
...
for lvl in np.arange(args.nlevel):
    ...
    # Open output ms file in casacore.tables
    ms_op = ct.table(ms_output, ack=False, readonly=False)

    vis_data = ms_op.getcol('DATA')
    print (f"MS Visibility Data shape: {vis_data.shape}") #66048,16,4 ::Nbaselines * Ntime, Nfrequency, NPol
    print (f"BIPP Visibility Data shape: {BIPP_Vis.shape}") #2, 924672 :: NLevel, Nbaseline*Nfrequency*Ntime
    # Maybe the continuity of data is different in the above two arrays, simple reshaping will not do the job. 
    # This is the issue, the data should be continuous in time first then in frequency, but our data streaming has continuity in frequency first, then time. 
    #BIPP_Vis = BIPP_Vis.reshape(args.nlevel, 899,4, -1) # FOR LOFAR GAUSS 4 DATASET nlevel, ntime, nchannel, nbaselines
    BIPP_Vis = BIPP_Vis.reshape(args.nlevel, 60, nChannel, -1)
    BIPP_Vis = np.transpose(BIPP_Vis, (0, 1, 3, 2)) # nlevel, ntime, nbaselines, nchannel
    print (BIPP_Vis.shape)

    vis_data[:, :, 0] = BIPP_Vis[lvl,:].reshape(BIPP_Vis.shape[1]*BIPP_Vis.shape[2], BIPP_Vis.shape[3])
    vis_data[:, :, 3] = BIPP_Vis[lvl,:].reshape(BIPP_Vis.shape[1]*BIPP_Vis.shape[2], BIPP_Vis.shape[3])
